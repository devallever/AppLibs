...
import org.xm.lib.image.loader.util.constant.CacheConstants;
...
public final class CacheDiskUtils implements CacheConstants {
    ...
    public static CacheDiskUtils getInstance(String cacheName, final long maxSize, final int maxCount) {
        if (StringUtils.isSpace(cacheName)) cacheName = "cacheUtils";
        File file = new File(App.context.getCacheDir(), cacheName);
    ...
    public void put(@NonNull final String key, final Bitmap value, final int saveTime) {
        realPutBytes(TYPE_BITMAP + key, ConvertUtils.bitmap2Bytes(value), saveTime);
    }
    ...
    public void put(@NonNull final String key, final Drawable value, final int saveTime) {
        realPutBytes(TYPE_DRAWABLE + key, ConvertUtils.drawable2Bytes(value), saveTime);
    }
