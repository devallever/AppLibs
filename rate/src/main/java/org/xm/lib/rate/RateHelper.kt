...
import org.xm.lib.core.util.Tool

object RateHelper {

    private var mAlertDialog: AlertDialog? = null

    fun createCommentDialog(activity: Activity?, listener: RateListener?): Dialog? {
        activity ?: return null
        val dialog = RateDialog(activity)
        dialog.setListener(listener)
        return dialog
    }

    fun create(activity: Activity?, listener: RateListener?): AlertDialog? {
        activity ?: return null
